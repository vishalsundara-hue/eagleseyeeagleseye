...
export function AlertsPanel() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="relative flex items-center gap-1.5 px-2.5 py-1 rounded-full glass hover:bg-muted transition"
        title="WhatsApp Alerts Dispatched"
      >
        <Bell className="size-3.5 text-primary" />
        <span className="text-primary hidden sm:inline">Alerts</span>
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-rose-500 text-foreground text-[10px] font-bold grid place-items-center animate-pulse">
            {unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[min(380px,90vw)] max-h-[70vh] overflow-hidden rounded-2xl glass-strong border border-border shadow-2xl z-50 flex flex-col">
          <div className="px-4 py-3 border-b border-border flex items-center gap-2">
            ...
            <div className="ml-auto text-[10px] text-muted-foreground">{alerts.length} sent</div>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 text-xs">
            <span className="font-semibold text-foreground truncate">Nurse {a.nurse.name}</span>
            <span className="text-muted-foreground">·</span>
            <span className="text-muted-foreground truncate flex items-center gap-1"><Phone className="size-3" />+{a.nurse.phone}</span>
            <span className="ml-auto text-[10px] text-muted-foreground shrink-0">{timeAgo(a.ts)}</span>
          </div>
          <div className="text-xs text-foreground/80 mt-1">
            <span className="text-rose-700 font-medium">{a.patientName}</span>
            <span className="text-muted-foreground"> · {a.patientId} · risk </span>
            <span className="text-amber-700 font-semibold">{a.risk}%</span>
          </div>
          <pre className="mt-2 whitespace-pre-wrap text-[11px] text-foreground/80 bg-black/30 border border-border rounded-lg p-2 max-h-32 overflow-y-auto font-sans">
{a.message}
          </pre>
